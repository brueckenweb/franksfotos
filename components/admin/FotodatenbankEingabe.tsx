...
export default function FotodatenbankEingabe() {
  ...
  const [tagSuche,             setTagSuche]             = useState("");
  const [tagSucheOffen,        setTagSucheOffen]        = useState(false);
              ...
              {allTags.length > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button
                      type="button"
                      onClick={() => { setTagSucheOffen((v) => !v); setTagSuche(""); }}
                  ...
                  {tagSucheOffen && (
                    <div className="mb-2">
                      <input
                        type="text"
                        value={tagSuche}
                        onChange={(e) => setTagSuche(e.target.value)}
                        placeholder="Tag suchen…"
                        autoFocus
                        className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-amber-500 placeholder-gray-600"
                      />
                      {/* Trefferliste */}
                      {(() => {
                        ...
                        const filtered = allTags.filter(
                          (t) =>
                            ...
                            (tagSuche === "" ||
                              t.name.toLowerCase().includes(tagSuche.toLowerCase()) ||
                              (t.groupName ?? "").toLowerCase().includes(tagSuche.toLowerCase()))
                        );
                        return filtered.length > 0 ? (
                          <div className="mt-1 max-h-32 overflow-y-auto bg-gray-800 border border-gray-700 rounded-lg">
                            {filtered.map((t) => (
                              <button
                                key={t.id}
                                type="button"
                                onClick={() => {
                                  setGalerieTags((prev) => [...prev, t]);
                                  setTagSuche("");
                                  setTagSucheOffen(false);
                                }}
                                className="w-full text-left px-3 py-1.5 text-xs text-gray-300 hover:bg-gray-700 hover:text-white flex items-center justify-between"
                              >
                                <span>
                                  {t.groupName && (
                                    <span
                                      style={{ color: t.groupColor ?? "#9ca3af" }}
